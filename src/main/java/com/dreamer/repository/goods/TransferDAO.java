...
public class TransferDAO extends HibernateBaseDAO<Transfer> {
	...
	public List<Transfer> searchEntity(SearchParameter<Transfer> t,
        ...
        if (Objects.nonNull(t.getEntity().getFromAgent())) {
            userByFromAgentCriteria.add(Restrictions.or(  Restrictions.like("realName",
                    t.getEntity().getFromAgent().getRealName(),MatchMode.ANYWHERE),  Restrictions.eq("agentCode",
                    t.getEntity().getFromAgent().getRealName())));
        }

        if (Objects.nonNull(t.getEntity().getToAgent())) {
            userByToAgentCriteria.add(Restrictions.or(  Restrictions.like("realName",
                    t.getEntity().getToAgent().getRealName(),MatchMode.ANYWHERE),  Restrictions.eq("agentCode",
                    t.getEntity().getToAgent().getRealName())));
...
//							}
	...
	public Transfer findById(java.lang.Integer id) {
		...
		try {
			Transfer instance = (Transfer) getCurrentSession().get(
					"com.dreamer.domain.mall.transfer.Transfer", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Transfer> findByExample(Transfer instance) {
		...
		try {
			List<Transfer> results = getCurrentSession()
					.createCriteria("com.dreamer.domain.mall.transfer.Transfer")
	...
	public List<Transfer> findAll(Integer uid,Integer limit) {
		try {
			String queryString = "from Transfer as t where t.userByFromAgent.id = :uid or t.userByToAgent.id = :uid order by t.updateTime desc ";
			Query queryObject = getCurrentSession().createQuery(queryString);
			queryObject.setParameter("uid",uid);
