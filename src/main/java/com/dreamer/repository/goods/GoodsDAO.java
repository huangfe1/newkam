...
public class GoodsDAO extends HibernateBaseDAO<Goods>{
	...
	public Goods findById(java.lang.Integer id) {
		...
		try {
			Goods instance = (Goods) getCurrentSession().get(
					"com.dreamer.domain.mall.goods.Goods", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Goods> findByExample(Goods instance) {
		...
		try {
			List results = getCurrentSession()
					.createCriteria("com.dreamer.domain.mall.goods.Goods")
	...
	public List<Goods> findByCategory(GoodsType gt, Integer cid) {
		try {
			String queryString = "from Goods as g where g.goodsType = :gt";
			if(cid!=null){
				queryString+="and g.category.id = :cid";
			}
			Query queryObject = getCurrentSession().createQuery(queryString);
			queryObject.setParameter("gt",gt);
			if(cid!=null){
				queryObject.setParameter("cid",cid);
			}
			return queryObject.list();
		} catch (RuntimeException re) {
			throw re;
