...
//            more = user.getRealName() + "购买" + goods.getName() + "货物" + amount + "" + goods.getSpec();//转账备注
        ...
        if (getAgentLevel().getName().contains(AgentLevelName.大区.toString())||getAgentLevel().getName().contains(AgentLevelName.董事.toString())||getAgentLevel().getName().contains(AgentLevelName.金董.toString())||getAgentLevel().getName().contains(AgentLevelName.联盟单位.toString())) {
            ...
            Agent agentWJC = null;//伟江初
            Double voucherWJC = 0.0;//伟讲出该拿的钱
...
//            }
            ...
            while (!parent.isMutedUser()) {//如果不是公司
                if(parent.getAgentCode().equals("zmz025608")){//如果是伟讲初
                    agentWJC=parent;
                }
                //如果是特权大区
...
//                        parent.getAccounts().increaseVoucher(voucher_temp, more);
                ...
                if (gac.getAgentLevel().getName().contains(AgentLevelName.联盟单位.toString())) {
                    ...
                    if(l_index!=0){         //如果有联盟
                        ...
                        getbackVoucher=getbackVoucher-getbackVoucher(1,amount);
                        voucher_temp+=getbackVoucher;
                    }else if(d_index!=0){   //如果有董事  金董事的返利减去董事的返利
                        getbackVoucher=getbackVoucher-getbackVoucher(2,amount);
                        voucher_temp+=getbackVoucher;
                    }else {
                        voucher_temp += getbackVoucher;//加上联盟的返利
                    }
                    voucherWJC=getbackVoucher/2.0;//伟讲初要拿联盟该拿的一半
                    if(toback==1){//退回货物
//                        parent.getAccounts().deductVoucherForBack(voucher_temp, more);
                        if(parent.getAgentCode().equals("zmz213821")||parent.getAgentCode().equals("zmz126786")){
                            if(parent.getAgentCode().equals("zmz126786")){//伟讲出拿好放凉的一半
                                //伟讲出吸收一半
                                if(!Objects.isNull(agentWJC)){
//                                    agentWJC.getAccounts().deductVoucherForBack(voucherWJC,more);
                                }
                            }
//                            parent.getAccounts().deductVoucherForBack(voucher_temp-voucherWJC, more);//只给一半联盟单位
                        }else {
//                            parent.getAccounts().deductVoucherForBack(voucher_temp, more);
                        }
                    }else {
...
//                        if(parent.getAgentCode().equals("zmz213821")||parent.getAgentCode().equals("zmz126786")){
                        if(parent.getAgentCode().equals("zmz213821")||parent.getAgentCode().equals("zmz126786")){
                            if(parent.getAgentCode().equals("zmz126786")){//伟讲出拿好放凉的一半
                                //伟讲出吸收一半
                                if(!Objects.isNull(agentWJC)){
//                                    agentWJC.getAccounts().increaseVoucher(voucherWJC,more);
                                }
                            }
//                            parent.getAccounts().increaseVoucher(voucher_temp-voucherWJC, more);
                        }else {
//                            parent.getAccounts().increaseVoucher(voucher_temp, more);
                        }
                    }
