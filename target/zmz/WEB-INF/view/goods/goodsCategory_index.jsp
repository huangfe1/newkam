        ...
        <div class="container-fluid">
            <div class="collapse navbar-collapse" id="top-navbar-collapse">
                <form class="navbar-form navbar-left" id="searchForm"
                      role="search" action="<c:url value='/goods/category/index.html'/>"
                      method="GET">
                    <input type="hidden" name="action" value="list"/>
                    <input type="hidden" name="resourceCode" value="${resourceCode }"/>
                    <div class="form-group">

                        <label class="">分类名称</label> <input type="text" value="${parameter.entity.name}" name="entity.name" id="name" autofocus class="typeahead form-control" placeholder="产品名称">
                        ...
                        <c:forEach items="${goodsType}" var="g">
                            <tr data-row="${g.id}">
                                <td style="padding-left:${g.type*10}px;"><a class="editBtn"
                                                                            href="<c:url value='/mall/goods/type/edit.html?id=${g.id}' /> ">${g.name}</a>
                                </td>
                                <td>${g.orderIndex}</td>
                                <td>${g.updateTime}</td>
                                <td><a class="btn btn-success default editBtn"
                                       href="<c:url value='/goods/category/edit.html?id=${g.id}' /> "><span
                                        class="glyphicon glyphicon-wrench" aria-hidden="true"></span>修改</a>
                                    <a class="ajaxLink btn btn-danger default" data-role="delete"
                                       href="<c:url value='/goods/category/remove.json?id=${g.id}' /> "><span
                                            class="glyphicon glyphicon-remove" aria-hidden="true"></span>删除</a>
                                    <a class="btn btn-success default editBtn"
                                       href="<c:url value='/goods/category/edit.html?parentType=${g.id}' /> "><span
                                            class="glyphicon glyphicon-plus" aria-hidden="true"></span>新增子等级</a>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="progress progress-striped active">
                        <div class="progress-bar" id="progressbar" role="progressbar" aria-valuenow="45"
                             aria-valuemin="0" aria-valuemax="100" style="width: 0%">
                            <span class="sr-only">45% Complete</span>
                        </div>
                    </div>
                    <div class="info" id="info">已上传：<span id="has_upload">0</span>MB 速度：<span id="upload_speed">0</span>KB/s
                    </div>
    ...
    $(function () {
        $(".addBtn").click(
            function (e) {
                ...
                e.stopPropagation();
                $("#myModal").load(
                    "<c:url value='/goods/category/edit.html'/>",
                    function (e) {
                        $('#myModal').modal({
                            backdrop: "static"
                        });
